...
function toEndpoint(row: typeof webhookEndpoints.$inferSelect): WebhookEndpoint {
  return {
    id: row.id,
...
async function createEndpoint(values: {
  workspaceId: string;
  name: string;
  description?: string;
  token: string;
...
async function findByToken(token: string): Promise<(WebhookEndpoint & { secretHash: string }) | null> {
...
async function deleteEndpoint(id: string, workspaceId: string): Promise<void> {
...
async function insertDelivery(values: {
  endpointId: string;
  workspaceId: string;
  deliveryKey: string;
...
async function listDeliveries(endpointId: string, workspaceId: string, limit = 50): Promise<WebhookDelivery[]> {
