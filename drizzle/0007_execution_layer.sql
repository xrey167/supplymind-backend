...
ALTER TABLE "execution_runs" ADD CONSTRAINT "execution_runs_plan_id_execution_plans_id_fk" FOREIGN KEY ("plan_id") REFERENCES "public"."execution_plans"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
CREATE INDEX "ep_workspace_created_idx" ON "execution_plans" USING btree ("workspace_id","created_at");--> statement-breakpoint
CREATE INDEX "er_plan_id_idx" ON "execution_runs" USING btree ("plan_id");--> statement-breakpoint
CREATE INDEX "er_workspace_created_idx" ON "execution_runs" USING btree ("workspace_id","started_at");
